...
test.beforeEach(async ({ page }) => {
  await page.goto('/')
  await page.locator('.DateField').scrollIntoViewIfNeeded()
...
test('date selection closes calendar and syncs native input', async ({ page }) => {
  ...
  await expect(page.locator('.DateFieldCalendar')).toHaveCount(0)
  const nativeValue = await page.locator('.DateField .Native').inputValue()
...
test('data-state="open" on root when calendar open', async ({ page }) => {
  await page.locator('.Trigger').click()
  await expect(page.locator('.DateField[data-state="open"]')).toHaveCount(1)
  await page.keyboard.press('Escape')
  await expect(page.locator('.DateField[data-state="idle"]')).toHaveCount(1)
})
